...
import { a as messaging, d as authStore, f as toastStore } from "../../chunks/api.js";
...
function Navbar($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		async function loadUnread() {
			if (!store_get($$store_subs ??= {}, "$authStore", authStore).authenticated) return;
			loadedUnread = true;
			unreadMessages = (await messaging.conversations().catch(() => null))?.conversations?.reduce((sum, item) => sum + (Number(item.unread_count) || 0), 0) || 0;
		}
		...
		$: if (store_get($$store_subs ??= {}, "$authStore", authStore).authenticated && !loadedUnread) loadUnread();
