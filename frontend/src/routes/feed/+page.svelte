<script>
  ...
  onMount(async () => {
    try {
      items = (await feed.list({ limit: 20 })).items || [];
    } finally {
      loading = false;
    }
  });
