<script>
  ...
  function selectFile(selected) {
    ...
    video.onloadedmetadata = () => { duration = Math.round(video.duration || 0); URL.revokeObjectURL(video.src); };
  ...
  async function publish() {
    ...
    try {
      const { video } = await videos.create({ title, description, difficulty, tags: [subject], language: 'ru' });
