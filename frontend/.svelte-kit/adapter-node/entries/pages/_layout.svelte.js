...
import { J as escape_html, K as attr, c as ensure_array_like, f as sanitize_props, g as unsubscribe_stores, h as store_get, m as spread_props, n as attr_class, p as slot } from "../../chunks/dev.js";
import { l as authStore, u as toastStore } from "../../chunks/api.js";
...
function Navbar($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		$: username = store_get($$store_subs ??= {}, "$authStore", authStore).user?.username || "arina_code";
		...
		$$renderer.push(`<!--]--></nav> <a class="user-card svelte-rfuq4y"${attr("href", `/profile/${username}`)}><img${attr("src", store_get($$store_subs ??= {}, "$authStore", authStore).user?.avatar_url || "https://i.pravatar.cc/120?img=47")} alt="" class="svelte-rfuq4y"/> <span class="svelte-rfuq4y"><strong class="user-card-name svelte-rfuq4y">${escape_html(store_get($$store_subs ??= {}, "$authStore", authStore).user?.display_name || "Арина Волкова")}</strong> <small class="user-card-handle svelte-rfuq4y">@${escape_html(username)}</small></span></a></aside> <nav class="mobile-nav svelte-rfuq4y"><!--[-->`);
