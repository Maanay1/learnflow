...
unless config_env() == :test do
  database_url =
    System.get_env("DATABASE_URL") ||
      "ecto://learnflow:devpassword123@localhost:5432/learnflow_dev"
  ...
  url_scheme =
    System.get_env(
      "URL_SCHEME",
      if(System.get_env("COOKIE_SECURE", "false") == "true", do: "https", else: "http")
    )
  ...
  config :learnflow, LearnflowWeb.Endpoint,
    ...
    url: [
      scheme: url_scheme,
      host: phx_host,
      port: if(url_scheme == "https", do: 443, else: port)
    ],
    check_origin: check_origins,
    secret_key_base:
      System.get_env(
        "SECRET_KEY_BASE",
        "dev_secret_key_base_change_me_for_production_64_bytes_minimum"
      ),
    server: true
...
config :learnflow, :openai, api_key: System.get_env("OPENAI_API_KEY")
